...
type SignQueryOptions = QueryOptions & { catalogueNamespace?: string; sortByFrequency?: boolean };
...
function mapOccurrence(row: Record<string, unknown>): SignOccurrence {
  ...
  return {
    ...
    recordScope: row.record_scope as SignOccurrence["recordScope"],
    positionIndex: Number(row.position_index),
    identificationStatus: row.identification_status as SignOccurrence["identificationStatus"],
    observedFormNote: row.observed_form_note as string | null,
    orientationNote: row.orientation_note as string | null,
    sign: hasSign
      ? {
          id: String(row.sign_id),
          stableId: String(row.sign_stable_id),
          catalogueNamespace: String(row.catalogue_namespace),
          catalogueCode: String(row.catalogue_code),
          visualLabel: String(row.visual_label),
          glyphSvg: row.glyph_svg as string | null,
        }
      : null,
  };
}

async function countAndRows<T>(
  countSql: string,
  rowSql: string,
  values: unknown[],
  pagination: ReturnType<typeof pageOptions>
): Promise<Page<T>> {
...
export async function listSites(options: QueryOptions = {}): Promise<Page<SiteSummary>> {
  ...
  if (options.search) {
    ...
    filters.push(
      `(s.canonical_name ILIKE $${values.length} OR s.modern_region ILIKE $${values.length} OR s.country ILIKE $${values.length})`
    );
  }
  const where = filters.length ? `WHERE ${filters.join(" AND ")}` : "";
  return countAndRows<SiteSummary>(
    `SELECT count(*)::text AS total FROM sites s ${where}`,
    `SELECT s.id, s.stable_id AS "stableId", s.record_scope AS "recordScope", s.status, s.canonical_name AS "canonicalName", s.modern_region AS "modernRegion", s.country FROM sites s ${where} ORDER BY s.canonical_name, s.stable_id LIMIT $${values.length + 1} OFFSET $${values.length + 2}`,
    values,
    pagination
  );
}
...
export async function getSite(id: string): Promise<(SiteDetail & { objectsCount: number; objects: ObjectSummary[] }) | null> {
  ...
  return {
    ...site,
    objectsCount: Number(countRes.rows[0]?.count ?? 0),
...
export async function listObjects(options: ObjectQueryOptions = {}): Promise<Page<ObjectSummary>> {
  ...
  if (options.search) {
    values.push(`%${options.search}%`);
    filters.push(
      `(o.stable_id ILIKE $${values.length} OR o.object_type ILIKE $${values.length} OR o.material ILIKE $${values.length})`
    );
  }
  const where = filters.length ? `WHERE ${filters.join(" AND ")}` : "";
  ...
  return countAndRows<ObjectSummary>(
    `SELECT count(*)::text AS total FROM objects o ${where}`,
    `${sql} ORDER BY o.stable_id LIMIT $${values.length + 1} OFFSET $${values.length + 2}`,
    values,
    pagination
  );
}
...
export async function getObject(id: string): Promise<(ObjectDetail & { inscriptions: InscriptionSummary[]; catalogueIdentifiers: CatalogueIdentifierSummary[] }) | null> {
  ...
  return {
    ...object,
    inscriptions: inscriptionsRes.rows,
    catalogueIdentifiers: identsRes.rows,
  };
}
...
export async function listInscriptions(options: InscriptionQueryOptions = {}): Promise<Page<InscriptionSummary>> {
  ...
  if (options.search) {
    values.push(`%${options.search}%`);
    filters.push(
      `(i.stable_id ILIKE $${values.length} OR o.stable_id ILIKE $${values.length} OR s.canonical_name ILIKE $${values.length} OR EXISTS (SELECT 1 FROM catalogue_identifiers ci WHERE ci.subject_type = 'object' AND ci.subject_id = o.id AND ci.identifier_text ILIKE $${values.length}) OR EXISTS (SELECT 1 FROM sign_sequences ss WHERE ss.inscription_id = i.id AND ss.source_transcription ILIKE $${values.length}))`
    );
  }
  const where = filters.length ? `WHERE ${filters.join(" AND ")}` : "";
  ...
  return countAndRows<InscriptionSummary>(
    `SELECT count(*)::text AS total FROM inscriptions i JOIN objects o ON o.id = i.object_id LEFT JOIN sites s ON s.id = o.site_id ${where}`,
    `${select} ORDER BY i.stable_id LIMIT $${values.length + 1} OFFSET $${values.length + 2}`,
    values,
    pagination
  );
}
...
export async function getInscription(id: string): Promise<(InscriptionDetail & { catalogueIdentifiers: CatalogueIdentifierSummary[]; sourceRelease?: { releaseLabel: string; sourceTitle: string; rightsSummary: string | null } }) | null> {
  ...
  for (const row of occurrenceResult.rows) {
    ...
    const entries = occurrencesBySequence.get(key) ?? [];
    entries.push(mapOccurrence(row));
    occurrencesBySequence.set(key, entries);
  }

  const sequences = sequencesResult.rows.map((sequence) => {
    const occurrences = occurrencesBySequence.get(sequence.id) ?? [];
    return {
      ...toSequence(
        sequence,
        occurrences.map((occurrence) =>
          occurrence.sign ? `${occurrence.sign.catalogueNamespace}:${occurrence.sign.catalogueCode}` : `[${occurrence.identificationStatus}]`
        )
      ),
      occurrences,
    };
  ...
  return {
    ...inscription,
    primarySequence: sequences[0] ? toSequence(sequences[0], sequences[0].tokens) : null,
    sequences,
    catalogueIdentifiers: identsRes.rows,
    sourceRelease: releaseRes.rows[0],
  };
}
...
export async function listSigns(options: SignQueryOptions = {}): Promise<Page<SignSummary>> {
  ...
  if (options.catalogueNamespace) {
    values.push(options.catalogueNamespace);
    filters.push(`sg.catalogue_namespace = $${values.length}`);
  }
  if (options.search) {
    values.push(`%${options.search}%`);
    filters.push(
      `(sg.stable_id ILIKE $${values.length} OR sg.catalogue_code ILIKE $${values.length} OR sg.visual_label ILIKE $${values.length})`
    );
  }
  const where = filters.length ? `WHERE ${filters.join(" AND ")}` : "";
  const orderBy = options.sortByFrequency
    ? `frequency DESC, sg.catalogue_namespace, sg.catalogue_code`
    : `sg.catalogue_namespace, sg.catalogue_code`;

  const sql = `SELECT sg.id, sg.stable_id AS "stableId", sg.record_scope AS "recordScope", sg.status, sg.catalogue_namespace AS "catalogueNamespace", sg.catalogue_code AS "catalogueCode", sg.visual_label AS "visualLabel", sg.glyph_svg AS "glyphSvg", COALESCE(occ.frequency, 0)::integer AS frequency FROM signs sg LEFT JOIN (SELECT sign_id, count(*)::integer AS frequency FROM sign_occurrences GROUP BY sign_id) occ ON occ.sign_id = sg.id ${where} ORDER BY ${orderBy}`;

  return countAndRows<SignSummary>(
    `SELECT count(*)::text AS total FROM signs sg ${where}`,
...
export async function getSign(id: string): Promise<SignDetail | null> {
  ...
  return {
    ...sign,
    occurrenceCount: Number(countRes.rows[0]?.count ?? 0),
...
export async function getSignSequence(id: string): Promise<SignSequenceDetail | null> {
  ...
  return {
    ...toSequence(
      sequence,
      mapped.map((occurrence) =>
        occurrence.sign ? `${occurrence.sign.catalogueNamespace}:${occurrence.sign.catalogueCode}` : `[${occurrence.identificationStatus}]`
      )
    ),
    inscription: sequence.inscription,
    occurrences: mapped,
  };
}

export async function getCorpusStatistics(): Promise<CorpusStatistics> {
  const statsRes = await databaseQuery<{
    total_inscriptions: string;
    research_inscriptions: string;
    total_objects: string;
    total_sites: string;
    total_sign_sequences: string;
    total_sign_occurrences: string;
    total_distinct_signs: string;
    total_catalogue_identifiers: string;
    total_sources: string;
  }>(
    `SELECT
      (SELECT count(*) FROM inscriptions WHERE record_scope = 'research')::text AS total_inscriptions,
      (SELECT count(*) FROM inscriptions WHERE record_scope = 'research')::text AS research_inscriptions,
      (SELECT count(*) FROM objects WHERE record_scope = 'research')::text AS total_objects,
      (SELECT count(*) FROM sites WHERE record_scope = 'research')::text AS total_sites,
      (SELECT count(*) FROM sign_sequences WHERE record_scope = 'research')::text AS total_sign_sequences,
      (SELECT count(*) FROM sign_occurrences WHERE record_scope = 'research')::text AS total_sign_occurrences,
      (SELECT count(*) FROM signs WHERE record_scope = 'research')::text AS total_distinct_signs,
      (SELECT count(*) FROM catalogue_identifiers WHERE record_scope = 'research')::text AS total_catalogue_identifiers,
      (SELECT count(*) FROM sources WHERE record_scope = 'research')::text AS total_sources`
  );

  const releasesRes = await databaseQuery<{
    release_label: string;
    delivery_version: string;
    provider_name: string;
    rights_summary: string | null;
  }>(
    `SELECT release_label, delivery_version, provider_name, rights_summary FROM corpus_releases WHERE record_scope = 'research' ORDER BY created_at`
  );

  const s = statsRes.rows[0];
  return {
    totalInscriptions: Number(s?.total_inscriptions ?? 0),
    researchInscriptions: Number(s?.research_inscriptions ?? 0),
    totalObjects: Number(s?.total_objects ?? 0),
    totalSites: Number(s?.total_sites ?? 0),
    totalSignSequences: Number(s?.total_sign_sequences ?? 0),
    totalSignOccurrences: Number(s?.total_sign_occurrences ?? 0),
    totalDistinctSigns: Number(s?.total_distinct_signs ?? 0),
    totalCatalogueIdentifiers: Number(s?.total_catalogue_identifiers ?? 0),
    totalSources: Number(s?.total_sources ?? 0),
    releases: releasesRes.rows.map((r) => ({
      releaseLabel: r.release_label,
      deliveryVersion: r.delivery_version,
      providerName: r.provider_name,
      rightsSummary: r.rights_summary,
    })),
  };
}
