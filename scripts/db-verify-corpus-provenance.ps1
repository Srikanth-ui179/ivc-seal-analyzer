...
try {
  Invoke-DockerCompose -ComposeArguments @('exec', '--no-TTY', 'postgres', 'psql', '-v', 'ON_ERROR_STOP=1', '-U', $DbUser, '-d', $DbName, '-f', '/workspace/db/scripts/corpus_provenance_verify.sql')
}
finally {
  Pop-Location
}
